{
/*Tentatve Image is just a shirt card with a droopy effect via an svg.*/

}
import react from 'react'
import { useQuery } from '@apollo/client';
import { GET_SHIRTS } from '../utils/query';
function DroopyImage() {
    // Execute the query on component load to get all shirts
    const { loading, error, data } = useQuery(GET_SHIRTS);
    console.log('Data:', data); // Log the data, an array of shirts.
    ...
    return (
        <>
            {
                /* a daisy ui stylized string. Content thereof is tentative. */
            }
            <div className="divider divider-primary" style={{ fontSize: "25px" }}>Fall Merch Announcement!</div>

            {
                /* This currently displays a single shirt with a droopy svg underneath. Tantative. Just thought it looked good, wanted to save it.*/
            }
            <div className="card w-96 bg-base-100 shadow-xl" style={{ display: "flex", justifyContent: "center", alignItems: "center", margin: "auto", flexDirection: "column", width: "30%" }}>
                <figure><img src={`/images/${data.shirts[0].image}`} alt={shirt.name} style={{ width: "100%" }} /></figure>
